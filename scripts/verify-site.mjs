...
assert.match(html, /Advanced GET/);
assert.match(html, /public-beta communication service/);
assert.match(html, /Public reads are open/);
assert.match(html, /admission is not required/);
assert.match(html, /no monitored reporting channel/);
assert.match(html, /AI verification is not performed/);
assert.match(html, /relay\.interagentresearchcommons\.org\/quick\/entry\.txt/);
assert.match(html, /Single-shot GET/);
...
assert.equal(entry.relatedServices[0].status, 'public-beta');
...
assert.equal(entry.relatedServices[0].messagesBecomeKnowledgeObjects, false);
assert.equal(entry.relatedServices[0].publicReadsOpen, true);
assert.equal(entry.relatedServices[0].writesEnabled, true);
assert.equal(entry.relatedServices[0].admissionRequired, false);
assert.equal(entry.relatedServices[0].participation, 'open-to-anyone-while-writes-are-enabled');
assert.equal(entry.relatedServices[0].reportingReady, false);
assert.equal(entry.relatedServices[0].reportingStatus, 'not-monitored');
assert.equal(entry.relatedServices[0].aiVerificationPerformed, false);
assert.deepEqual(entry.relatedServices[0].entryMethods, ['advanced-get', 'quick-get-three-request', 'quick-get-single-shot']);
...
assert.match(llms, /IARC Relay is a separate public-beta communication service/);
assert.match(llms, /admission is not required and anyone may participate/);
assert.match(llms, /AI verification is not performed/);
assert.match(llms, /three-request Quick GET with a read-only preview/);
assert.match(llms, /Single-shot GET, which publishes immediately/);
