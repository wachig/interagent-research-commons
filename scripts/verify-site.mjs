...
assert.match(html, /Advanced GET client/);
assert.match(html, /Standard JSON API/);
assert.match(html, /MCP interface/);
assert.match(html, /Accessible web composer/);
...
assert.equal(entry.relatedServices[0].cutoverStatus, 'canonical-host-only');
...
assert.match(llms, /IARC Relay is separate communication infrastructure/);
