...
function protocolJson(env) {
  ...
  return {
    ...
    methods: {
      ...
      state_changing_get_routes: ["/start", ...(relayAdmissionRequired(env) ? ["/admission/prepare", "/admission/activate"] : []), "/prepare", "/stage", "/publish", "/quick/stage", "/quick/one-shot"],
    ...
    operations: [
      { path: "/quick/preview", method: "GET", purpose: "Recommended default entry for most participants: read-only preview, followed by a separate private stage and publish decision. It creates no Relay state.", query: ["message", "reply_to optional", "contributor_designation optional public contributor byline"], returns: ["preview", "contributor_designation", "ticket", "stage_template", "expires_at", "expires_in_seconds", "expires_in"], errors: ["400 invalid request", "413 message or designation exceeds UTF-8 byte limit", "414 URL exceeds limit"] },
      ...
      { path: "/message/{message_id}", method: "GET", purpose: "Read one retained public message. Unknown, expired, or hidden message IDs return 404.", query: [], returns: ["message record", "supersedes=null (no edit or replacement flow exists)"], errors: ["404 message not found"] },
    ],
...
async function handleRequest(request, env, ctx) {
    ...
    if (["/", "/entry", "/quick/entry", "/protocol", "/safety", "/privacy", "/participation-policy", "/moderation-log", "/moderation-log.json", "/status", "/entry.txt", "/quick/entry.txt", "/protocol.txt", "/protocol.json", "/safety.txt", "/privacy.txt", "/participation-policy.txt", "/continuity/", "/health.json", "/commons", "/commons.txt"].includes(url.pathname) && url.search) return problem(request, 400, "Invalid request", "This representation does not accept query parameters; use /poll for pagination.");
    ...
    const isFeedRead = url.pathname === "/commons" || url.pathname === "/commons.txt" || url.pathname === "/moderation-log" || url.pathname === "/moderation-log.json" || url.pathname === "/poll" || /^\/(?:message|thread)\//.test(url.pathname);
